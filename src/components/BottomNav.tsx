...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-2.5 sm:bottom-4 inset-x-0 mx-auto max-w-md px-3 sm:px-4 z-50 pointer-events-none transition-all duration-300 pb-[env(safe-area-inset-bottom,0px)]">
      <nav className="pointer-events-auto glass-dock rounded-full px-2 sm:px-3 py-1.5 sm:py-2 flex items-center justify-between shadow-[0_20px_50px_rgba(11,51,53,0.5)] border border-white/25">
          ...
          {navItemsLeft.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-0.5 transition-all duration-300 w-11 sm:w-13 h-10 sm:h-11 rounded-2xl relative active:scale-90 ${
                  isActive
        ...
        <div className="relative -top-4 sm:-top-5 flex justify-center px-1">
          <button
            onClick={() => router.push("/cart")}
            className={`w-11.5 h-11.5 sm:w-13 sm:h-13 rounded-full flex items-center justify-center transition-all duration-300 active:scale-90 relative ${
              activeTab === "cart"
